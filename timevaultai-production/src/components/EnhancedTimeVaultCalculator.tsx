...
// Enhanced crypto asset definitions with expanded support
...
export default function EnhancedTimeVaultCalculator() {
  // Enhanced API Hooks with improved error handling and retry mechanisms
  const {
    data: cryptoData,
  ...
  const { trackEvent, trackCalculation, trackPremiumInterest, trackPerformance } = useEnhancedAnalytics();
  const { measureOperation } = usePerformanceMonitoring();
  const { portfolioMode, setPortfolioMode, addToPortfolio } = useAdvancedCalculations();
  ...
  // Transform crypto data into enhanced assets array with performance monitoring
  const cryptoAssets = useMemo((): CryptoAsset[] => {
    ...
    }, 'crypto_assets_transformation');
  }, [cryptoData, measureOperation]);
  ...
  // Track calculations and engagement
  useEffect(() => {
    if (calculationResult) {
      ...
      trackCalculation({
        asset: calculationResult.asset.symbol,
        amount: calculationResult.amount,
        usdValue: calculationResult.usdValue,
        metals: {
          gold: calculationResult.metals.gold.amount,
          silver: calculationResult.metals.silver.amount,
          platinum: calculationResult.metals.platinum.amount,
          palladium: calculationResult.metals.palladium.amount
        },
        timeValue: calculationResult.timeValue.hours
      });

      // Show premium prompt for high-value calculations or frequent users
      if (calculationCount >= 3 || calculationResult.usdValue > 10000) {
        setShowPremiumPrompt(true);
      }
    }
  }, [calculationResult, calculationCount, trackCalculation]);
  ...
  const handleAssetChange = useCallback((assetId: string) => {
    ...
    trackEvent('asset_selected', { asset: assetId });
  }, [trackEvent]);

  // Handle amount change
  const handleAmountChange = useCallback((value: string) => {
    setAmount(value);
    trackEvent('amount_changed', { amount: parseFloat(value) || 0 });
  }, [trackEvent]);

  // Handle premium interest
  const handlePremiumInterest = useCallback((source: string) => {
    trackPremiumInterest(source, {
      calculation_value: calculationResult?.usdValue || 0,
      calculation_count: calculationCount
    });
    setShowPremiumPrompt(true);
  }, [trackPremiumInterest, calculationResult?.usdValue, calculationCount]);

  // Data freshness indicator
  const isDataFresh = useMemo(() => {
    const now = new Date();
    const cryptoAge = now.getTime() - cryptoUpdated.getTime();
    const metalsAge = now.getTime() - metalsUpdated.getTime();
    return cryptoAge < 300000 && metalsAge < 600000; // 5min crypto, 10min metals
  }, [cryptoUpdated, metalsUpdated]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#001F3F] to-[#003366] py-4 px-4 sm:py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 text-xs sm:text-sm text-gray-400 mb-4">
            <div className="flex items-center">
              ...
              Real-time prices ({supportedAssets} assets)
            </div>
            <div className="flex items-center">
              {cryptoHealth === 'healthy' ? (
                <CheckCircle className="w-4 h-4 mr-1 text-green-400" />
              ) : cryptoHealth === 'degraded' ? (
          ...
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            {calculationResult && (
              <div className="space-y-4 sm:space-y-6">
                ...
                {/* Premium Upsell */}
                {(calculationResult.usdValue > 5000 || showPremiumPrompt) && (
                  <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-lg p-4 sm:p-6 border border-purple-500/30">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-base sm:text-lg font-bold text-white">Premium Insights Available</h3>
                        <p className="text-sm text-gray-300">Get AI-powered analysis and advanced tools</p>
                      </div>
                      <Crown className="w-6 h-6 sm:w-8 sm:h-8 text-[#D4AF37]" />
                    </div>
                    <button
                      onClick={() => handlePremiumInterest('calculator_upsell')}
        ...
        <div className="text-center text-gray-400 text-sm">
          ...
          <p className="mt-2">
            Last updated: Crypto {cryptoUpdated.toLocaleTimeString()}, Metals {metalsUpdated.toLocaleTimeString()}
          </p>
        </div>
      </div>
    </div>
...
return (
  <div className="min-h-screen bg-gradient-to-br from-[#001F3F] to-[#003366] py-4 px-4 sm:py-8">
    <div className="max-w-7xl mx-auto">{/* Increased max width and ensured proper containment */}
      {/* Header Section */}
      <div className="text-center mb-8 sm:mb-12">
        <div className="flex items-center justify-center mb-4 sm:mb-6">
          <div className="bg-[#D4AF37] p-3 sm:p-4 rounded-full mr-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 text-xs sm:text-sm text-gray-400 mb-4">{/* Made responsive */}
          <div className="flex items-center">
            <Zap className="w-4 h-4 mr-1 text-[#D4AF37]" />
            Real-time prices
          </div>
          <div className="flex items-center">
            ...
            {isDataFresh ? 'Live data' : 'Cached data'}
          </div>
          <div className="flex items-center">
            <Star className="w-4 h-4 mr-1 text-[#D4AF37]" />
            WCAG accessible
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">{/* Enhanced responsive grid for 12 assets */}
                {cryptoAssets.map((asset) => (
                  <button
                    key={asset.id}
                    onClick={() => handleAssetChange(asset.id)}
    ...
    {calculationResult && (
      <div className="space-y-4 sm:space-y-6">
        ...
        {/* Precious Metals */}
        <div className="grid grid-cols-2 gap-3 sm:gap-4">{/* Responsive grid spacing */}
          <div className="bg-white/5 rounded-lg p-3 sm:p-4 border border-white/10">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs sm:text-sm text-gray-300">Gold</span>
              <span className="text-lg">🥇</span>
